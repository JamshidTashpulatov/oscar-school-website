...
var _mouseFollower = require("mouse-follower");
var _mouseFollowerDefault = parcelHelpers.interopDefault(_mouseFollower);
...
(0, _mouseFollowerDefault.default).registerGSAP((0, _gsap.gsap));
...
requestAnimationFrame(raf);
// Mouse Follower
const cursor = new (0, _mouseFollowerDefault.default)();
// Custom JS
...
(0, _gsap.gsap).from(".intro__center-line span", 1.8, {
    y: 200,
    ease: "power4.out",
    skewY: 7,
    stagger: {
...
(0, _gsap.gsap).from(".hr__wrapper .hr__wrappper__item", {
    scrollTrigger: {
...
(0, _gsap.gsap).from(".images__img__container", {
    scrollTrigger: {
...
function hide(elem) {
    (0, _gsap.gsap).set(elem, {
        autoAlpha: 0
    });
}
document.addEventListener("DOMContentLoaded", function() {
    (0, _gsap.gsap).registerPlugin((0, _scrollTrigger.ScrollTrigger));
    (0, _gsap.gsap).utils.toArray(".section__title").forEach(function(elem) {
    ...
    (0, _gsap.gsap).utils.toArray(".block__title").forEach(function(elem) {
        hide(elem); // assure that the element is hidden when scrolled into view
        (0, _scrollTrigger.ScrollTrigger).create({
            trigger: elem,
            // markers: true,
    ...
    (0, _gsap.gsap).utils.toArray(".footer__bottom p").forEach(function(elem) {
        hide(elem);
        (0, _scrollTrigger.ScrollTrigger).create({
            trigger: elem,
            onEnter: function() {
    ...
    (0, _gsap.gsap).utils.toArray(".experience span").forEach(function(elem) {
        hide(elem); // assure that the element is hidden when scrolled into view
        (0, _scrollTrigger.ScrollTrigger).create({
            trigger: elem,
            // markers: true,
    ...
    (0, _gsap.gsap).utils.toArray(".course-img").forEach(function(elem) {
        hide(elem); // assure that the element is hidden when scrolled into view
        (0, _scrollTrigger.ScrollTrigger).create({
            trigger: elem,
            // markers: true,
    ...
    (0, _gsap.gsap).utils.toArray(".card__top__item span").forEach(function(elem) {
        hide(elem); // assure that the element is hidden when scrolled into view
        (0, _scrollTrigger.ScrollTrigger).create({
            trigger: elem,
            // markers: true,
    ...
    (0, _gsap.gsap).utils.toArray(".card__center").forEach(function(elem) {
        hide(elem); // assure that the element is hidden when scrolled into view
        (0, _scrollTrigger.ScrollTrigger).create({
            trigger: elem,
            // markers: true,
    ...
    (0, _gsap.gsap).utils.toArray(".short__bio img").forEach(function(elem) {
        hide(elem); // assure that the element is hidden when scrolled into view
        (0, _scrollTrigger.ScrollTrigger).create({
            trigger: elem,
            // markers: true,
    ...
    (0, _gsap.gsap).utils.toArray(".card__bottom p").forEach(function(elem) {
        hide(elem); // assure that the element is hidden when scrolled into view
        (0, _scrollTrigger.ScrollTrigger).create({
            trigger: elem,
            // markers: true,
    ...
    (0, _gsap.gsap).utils.toArray(".short__bio p").forEach(function(elem) {
        hide(elem); // assure that the element is hidden when scrolled into view
        (0, _scrollTrigger.ScrollTrigger).create({
            trigger: elem,
            // markers: true,
    ...
    (0, _gsap.gsap).utils.toArray(".card_hr .hr__wrappper__item").forEach(function(elem) {
        hide(elem); // assure that the element is hidden when scrolled into view
        (0, _scrollTrigger.ScrollTrigger).create({
            trigger: elem,
            // markers: true,
...
const boxes = (0, _gsap.gsap).utils.toArray(".animation h4");
...
function horizontalLoop(items, config) {
    ...
    }), length = items.length, startX = items[0].offsetLeft, times = [], widths = [], xPercents = [], curIndex = 0, pixelsPerSecond = (config.speed || 1) * 100, snap = config.snap === false ? (v)=>v : (0, _gsap.gsap).utils.snap(config.snap || 1), totalWidth, curX, distanceToStart, distanceToLoop, item, i;
    (0, _gsap.gsap).set(items, {
        // convert "x" to "xPercent" to make things responsive, and populate the widths/xPercents Arrays to make lookups faster.
        xPercent: (i, el)=>{
            let w = widths[i] = parseFloat((0, _gsap.gsap).getProperty(el, "width", "px"));
            xPercents[i] = snap(parseFloat((0, _gsap.gsap).getProperty(el, "x", "px")) / w * 100 + (0, _gsap.gsap).getProperty(el, "xPercent"));
            return xPercents[i];
        }
    });
    (0, _gsap.gsap).set(items, {
    ...
    totalWidth = items[length - 1].offsetLeft + xPercents[length - 1] / 100 * widths[length - 1] - startX + items[length - 1].offsetWidth * (0, _gsap.gsap).getProperty(items[length - 1], "scaleX") + (parseFloat(config.paddingRight) || 0);
    for(i = 0; i < length; i++){
        item = items[i];
        curX = xPercents[i] / 100 * widths[i];
        distanceToStart = item.offsetLeft + curX - startX;
        distanceToLoop = distanceToStart + widths[i] * (0, _gsap.gsap).getProperty(item, "scaleX");
        tl.to(item, {
    ...
    function toIndex(index, vars) {
        ...
        if (time > tl.time() !== index > curIndex) {
            ...
            vars.modifiers = {
                time: (0, _gsap.gsap).utils.wrap(0, tl.duration())
            };
...
document.addEventListener("click", function() {
    if (!isAudioAllowed) audio.play().then(()=>{
        isAudioAllowed = true;
    }).catch((error)=>{
        console.error("Failed to play audio:", error);
    });
});
let sections = (0, _gsap.gsap).utils.toArray(".panel");
(0, _gsap.gsap).to(sections, {
...
},{"@studio-freight/lenis":"ggVJc","gsap":"fPSuC","gsap/ScrollTrigger":"7wnFk","mouse-follower":"cUjaw","gsap/ScrollToPlugin":"9xJDW","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"ggVJc":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"cUjaw":[function(require,module,exports) {
/*!
 ...
 */ var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "default", ()=>MouseFollower);
var MouseFollower = /*#__PURE__*/ function() {
   ...
   */ ;
    function MouseFollower(options) {
        ...
        /** @type {MouseFollowerOptions} **/ this.options = Object.assign({}, {
            el: null,
    ...
    _proto.bind = function bind() {
        ...
        this.event.mouseover = function(e) {
            for(var target = e.target; target && target !== _this.container; target = target.parentNode){
                ...
                for(var state in _this.options.stateDetection)if (target.matches(_this.options.stateDetection[state])) _this.addState(state);
        ...
        this.event.mouseout = function(e) {
            for(var target = e.target; target && target !== _this.container; target = target.parentNode){
                if (e.relatedTarget && target.contains(e.relatedTarget)) break;
                for(var state in _this.options.stateDetection)if (target.matches(_this.options.stateDetection[state])) _this.removeState(state);
                if (_this.options.dataAttr) {
                    var params = _this.getFromDataset(target);
        ...
        if (this.options.hideOnLeave) this.container.addEventListener("mouseleave", this.event.mouseleave, {
            passive: true
        });
        if (this.options.visible) this.container.addEventListener("mouseenter", this.event.mouseenter, {
            passive: true
        });
        ...
        if (this.options.visible) this.container.addEventListener("mousemove", this.event.mousemoveOnce, {
            passive: true,
            once: true
        });
   ...
   */ ;
    _proto.toggle = function toggle(force) {
        if (force === true || force !== false && !this.visible) this.show();
        else this.hide();
    } /**
   ...
   */ ;
    _proto.toggleState = function toggleState(state, force) {
        if (force === true || force !== false && !this.el.classList.contains(state)) this.addState(state);
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["j9r0q","ebWYT"], "ebWYT", "parcelRequire2524")
